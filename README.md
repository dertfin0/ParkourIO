...
Без указания лимита будет показано до 100 последних наказаний. Отображаются наказания в следующем формате:  
`[Тип] Причина`
...
### Создание летающего текста (TextDisplay)
Для упрощённого создания TextDisplay на сервер добавлена `/tdu` (text-display-util).

Использование:
`/tdu remove-closest` - удаляет ближайший `TextDisplay`  
`/tdu spawn <aligment> <billboard> <text>` - создать новый `TextDisplay`, где  
`aligment` - привязка текста (по центру, к левому краю, к правому краю),  
`billboard` - Положение голограммы (фиксированное, поворот по вертикальной оси, поворот по горизонтальной оси)
`text` - Текст в формате MiniMessage с использованием `<br>` для переноса строки
